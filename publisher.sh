...
# for tiff in *.tif; do
# 	scp ./$tiff kotkanen@geoserver.hel.fi:/home/kotkanen/geoserver-data/$WORKSPACE/$tiff
# done
...
Set up each of the files (probably only 1 at a time) as a WMS-layer on the geoserver
...
for f in *.tif; do
	...
	curl -u $USER:$PASSWORD -v -XPUT -H "Content-type:text/plain" -d "file:///srv/geoserver/data/data/kotkanen/$WORKSPACE/$f" http://geoserver.hel.fi/geoserver/rest/workspaces/$WORKSPACE/coveragestores/$f/external.geotiff
